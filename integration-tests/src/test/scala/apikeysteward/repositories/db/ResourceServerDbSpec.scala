...
class ResourceServerDbSpec
  ...
  "ResourceServerDb on delete" when {
    ...
    "there are no rows in the DB" should {
      ...
      "return Left containing ResourceServerNotFoundError" in {
        val result = (for {
          ...
          res <- resourceServerDb.delete(publicTenantId_1, publicResourceServerId_1)
        } yield res).transact(transactor)
    ...
    "there is a row in the DB for different publicTenantId" should {
      ...
      "return Left containing ResourceServerNotFoundError" in {
        val result = (for {
          ...
          res <- resourceServerDb.delete(publicTenantId_2, publicResourceServerId_1)
        } yield res).transact(transactor)
    ...
    "there is a row in the DB with different publicResourceServerId" should {
      ...
      "return Left containing ResourceServerNotFoundError" in {
        val result = (for {
          ...
          res <- resourceServerDb.delete(publicTenantId_1, publicResourceServerId_2)
        } yield res).transact(transactor)
    ...
    "there is a row in the DB with given publicResourceServerId" should {
      ...
      "return deleted entity" in {
        ...
        result.asserting(_ shouldBe Right(resourceServerEntityRead_1))
      }
      ...
      "delete this row from the DB and leave others intact" in {
        ...
        result.asserting { res =>
          res should contain theSameElementsAs List(
            resourceServerEntityRead_2.copy(tenantId = tenantDbId_1),
            resourceServerEntityRead_3.copy(tenantId = tenantDbId_1)
          )
        }
      }
    }
