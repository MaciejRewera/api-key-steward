...
sealed abstract class Auth0Error(override val message: String) extends CustomError

object Auth0Error {

  sealed abstract class Auth0LoginError(message: String) extends Auth0Error(message)
  ...
  case class Auth0UpstreamErrorResponse(override val statusCode: Int, errorMessage: String)
      extends Auth0Error(message = s"An error occurred when calling Auth0 API: $errorMessage")
      with UpstreamErrorResponse
